...
import pickle # Barry
cache_folder = f'{PROJECT_ROOT}\\pickle_cache'
...
def get_pickle_cache(request):
    filepath = request.param['filepath']
    if os.path.exists(filepath):
        with open(filepath, 'rb') as f:
            data = pickle.load(f)
            raw_els = data.get('cached_html')
            if isinstance(raw_els, list):
                elements = []
                for el in raw_els:
                    selenium_el = HtmlAttributes().get_element(el)
                    elements.append(selenium_el)

                yield elements
                return elements
            else:
                el = HtmlAttributes().get_element(raw_els)
                yield el
                return el
    else:
        my_el = {}
        ...
        for att in funcs:
            my_el[att] = True
        my_el['is_displayed'] = False
        attributes = HtmlAttributes().get_element(my_el)
        yield attributes
        return attributes
